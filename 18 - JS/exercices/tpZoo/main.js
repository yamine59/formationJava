...
ajouter.addEventListener("click", (e) => {
    ...
    if (selectedEspece === "Lion") {
        animal = new Lion(nom.value, age.value, selectedEspece, cri.value);
    ...
    card.innerHTML = `
        <div class="flex items-center">
            <h5 class="text-lg font-bold text-blue-500">${animal.nom}</h5>
        </div>
        <p class="text-gray-700 text-sm mt-2">Age: ${animal.age}</p>
        <p class="text-gray-700 text-sm">Espèce: ${animal.espece}</p>
        <p class="text-gray-700 text-sm">Cri: ${animal.cri}</p>
    ...
    document.querySelector(`#${btnId}`).addEventListener("click", (e) => {
        ...
        criCard.innerHTML = `<p class="font-semibold text-lg">${animal.crier()}</p>`;
